 ...
 * PrayerLockWidget 1 to 3 + PrayerLockWidget5, the two-column day list), the
 * standard dark pair (PrayerWidgetDark + PrayerWidgetDarkMedium), the extras
 * light set (ExtrasWidget + ExtrasWidgetMedium + ExtrasLockWidget 1 to 3 +
 * ExtrasLockWidget4, the one-column day list), and the extras dark pair
...
const pushScheduleTimelines = async (schedule: ScheduleType): Promise<void> => {
  ...
  try {
    ...
    if (schedule === ScheduleType.Standard) {
      ...
      lock.PrayerLockWidget5.updateTimeline(lightEntries);
      ...
      lock.ExtrasLockWidget4.updateTimeline(lightEntries);
