...
/**
 * Instants inside and outside the season, including 00:30 and 23:59:41 BST in Ramadan 1445
 * (Umm al-Qura: 11 March to 9 April 2024, BST from 31 March)
 */
const INSTANTS = [
  { label: 'Ramadan 1447, GMT', iso: '2026-03-10T12:00:00Z', inSeason: true },
  { label: "the pre-Ramadan window in Sha'ban 1447", iso: '2026-02-15T12:00:00Z', inSeason: true },
  { label: '00:30 BST on 5 April 2024, Ramadan 1445', iso: '2024-04-04T23:30:00Z', inSeason: true },
  { label: '23:59:41 BST on 4 April 2024, Ramadan 1445', iso: '2024-04-04T22:59:41Z', inSeason: true },
...
describe('isRamadan with a working Intl (control)', () => {
  it.each(INSTANTS)('reads $label as inSeason=$inSeason', ({ iso, inSeason }) => {
...
describe('isRamadan when Intl fails', () => {
  it.each(INSTANTS)('is false, without throwing, at $label when the calendar is rejected', ({ iso }) => {
    jest.setSystemTime(new Date(iso));
  ...
  it.each(INSTANTS)('is false, without throwing, at $label when formatting fails', ({ iso }) => {
    jest.setSystemTime(new Date(iso));
