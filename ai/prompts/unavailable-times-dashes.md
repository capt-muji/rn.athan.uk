...
1.27.12 on `feat/audit-71-dashes`, not merged), built the owner's rulings on the screenshots:
...
- R14: the bar and the "ago" badge measure only from the prayer directly above next, and hide when it is dashed
- R11: a waiting `--:--` countdown is named `...` on both schedules; the overlay names its prayer
