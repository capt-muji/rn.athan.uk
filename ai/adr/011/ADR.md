...
6. **Design**: "Flat royal" — solid `COLORS.navigation.rootBackground` card, hero `#e6f0ff` (success-white brightened) bold 26 pt (= `TEXT.sizeLarge`, so worst-case `24h` clears the edges), absolute time `COLORS.text.secondary`, prayer name as an uppercase letter-spaced eyebrow in a widget-only periwinkle (`rgba(163, 185, 252, 0.62)` — an indigo lean so it fades into the card without going purple), and a `Sat · London` footer nudged just under the absolute time's color (`rgba(157, 188, 246, 0.48)`; day derived from `dateLabel.split(',')[0].slice(0,3)`; Hijri yields the 3-letter month). Lock Screen rectangular pairs the countdown with the prayer name (`Magrib · 9m`) and puts the absolute `HH:mm` below; the circular face (a duplicate countdown) is retired — kept in `supportedFamilies` but rendered blank, because iOS keeps user-placed accessory instances alive after their family leaves the configuration and freezes them on their last render.
