...
- **The Lock Screen carries THREE compositions, six kinds, and one muted element (2026-09-25, session 23).** Layout 1 is name + absolute time with the ticking countdown beneath, Layout 2 is name + absolute time on one centred line, Layout 3 is name + ticking countdown on one centred line; each registers for both schedules, so `widgets/LockPrayerWidget.tsx` holds three `'widget'`-directive functions and `widgetContract.test.ts` pins that count. A new kind is four edits, never one: the layout function, its `createWidget` export, an `app.json` entry, an `updateTimeline` call in `stores/widget.ts`, plus the kind in `shared/__mocks__/widgets/LockPrayerWidget.ts` or three store suites fail on the missing mock. Type scale is name 17, secondary 14, eyebrow 11, set by the owner on the XS (+50% was too big, −20% from there was right). **Only the absolute time is muted and only the prayer name is bold**, so weight carries the hierarchy opacity used to: the old `WHITE_SECONDARY` was named for its tier rather than its reason and had spread to five elements the owner never wanted faded, hence `WHITE_MUTED`. Its `rgba(255, 255, 255, 0.6)` must stay in `widgetContract.test.ts`'s `widgetSpecific` list.
