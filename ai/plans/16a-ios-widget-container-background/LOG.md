...
## 52. The bake experiment, GO given — scope widened by the owner to both platforms

Owner (2026-09-20, lifting the gate): "I want to do the blur first... the non-PNG first, on both
platforms, and then converting that to PNG and seeing if we see the same performance or an
improvement in performance. Go." Platform fact shaping it: Glance cannot blur, so the non-PNG
phase exists only on iOS; Android's orbs can only ever be the baked form. Running as:
Phase A — iOS runtime orbs restored byte-for-byte from `10189469` (1.27.317, the last orb state:
OrbLight, NEBULA constants, their renderer/contract pins; suite 43 passed on the restore),
experimental build 1.27.323, the XS flash test + counter re-measured with NO patch in the tree.
Phase B — orbs baked into the card PNGs (iOS translucent base as two new assets replacing the
runtime layer; Android's existing dark card PNGs regenerated with the orbs baked in, layout
untouched), same measurements, owner judges look and numbers on both platforms. Everything on
`experiment/16a-orb-png-study`; nothing merges until the owner rules. NARROWED minutes later by the
owner: "do it on iOS only, because that's where we had the issues. Android already will be using
PNGs." Android keeps its flat cards untouched; both phases and all measurements are iOS-only.

## 53. The experiment ran, and the FINAL ruling: completely flat, both platforms

Phase A (runtime orbs restored from `10189469`, build 1.27.323): the owner's placement was
CLEAN — but the identical configuration had flashed earlier in the A/B, so the flash is a
marginal first-render race, and single samples cannot rank blur against PNG. Phase B (baked
PNG, build 1.27.324): three iterations to get it working — (1) an alpha-compositing bug in the
bake (PIL paste-with-mask wipes the base; fixed with alpha_composite then one clip), (2)
SwiftUI's `Image(assetName:)` does not load loose bundle PNGs, fixed by generating a proper
`.xcassets` catalog wired into the extension's resources. Result: the nebula rendered, small
placements clean, ONE medium placement showed a 2-second black-then-load; no runtime baseline
existed for the medium, so the comparison stayed inconclusive at that size. The owner then
ruled FINALLY: "completely, completely remove the orbs. Anything to do with the orbs, just keep
it flat for both Android and iOS, just keep it completely, completely flat." The experiment
branch `experiment/16a-orb-png-study` was deleted without merging; its artifacts removed; the
ios project regenerated by prebuild + pod install (verified zero experiment wiring remains);
`uat-2` was already flat from 1.27.318. The orb question is CLOSED FOR GOOD. The look design
continues flat per the owner's next session.
