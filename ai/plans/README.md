...
| 21 | 22. Dependency freshness sweep: re-measure every non-SDK package and batch what has moved | `ai/plans/README.md` "How a dependency upgrade is split into sessions" | not written yet | **NOT PLANNED, and this is the NEXT session** (owner 2026-09-26). Needs NO device: it is a desk session of registry reads, bumps and `yarn validate`, and the Find X8 has gone back to the owner. Row 18 (SDK 58 stable) is the only other unblocked row and it waits on npm, so this runs first and clears the deck before SDK 58 stable lands and jumps the queue | | 20 (session 21 must be DONE, because it is what makes this a sweep rather than the first pass) |
