...
| 23 | 24. Lock Screen widgets: the whole day's list, in two styles (iOS only) | `ai/plans/24-lock-widget-full-day-list/PLAN.md` section 2 | `ai/plans/24-lock-widget-full-day-list/PLAN.md` | READY | `5144ae93` | nothing. Four new kinds: one-column and two-column styles, both schedules. Absolute `HH:mm` only, nothing ticking; the day rollover already comes from `resolveDisplayDate` through the builder, so no layout does date maths. **A 2026-09-25 attempt built this as a systemLarge HOME widget by misreading the brief and was reverted in full** |
