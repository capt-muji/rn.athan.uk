...
| 23 | 24. Lock Screen widgets: the whole day's list (iOS only) | `ai/plans/24-lock-widget-full-day-list/PLAN.md` section 2 | `ai/plans/24-lock-widget-full-day-list/PLAN.md` | EXECUTED (audited 2026-09-25, PASS on the code at `21560164`; stays EXECUTED until the owner has looked at both faces on the XS, per the plan's section 9) | `273abe96` | nothing. **Rescoped by the owner on 2026-09-25 from four kinds to two:** `ExtrasLockWidget4` lists the extras day in one column (4 rows, 5 on Fridays) and `PrayerLockWidget5` splits the standard day 3 and 3, first half left. One style per schedule, not both. Absolute `HH:mm` only, nothing ticking; the day rollover already comes from `resolveDisplayDate` through the builder, so no layout does date maths. Also drops the unreachable `try`/`catch` from all five lock layouts (owner). **A 2026-09-25 attempt built this as a systemLarge HOME widget by misreading the brief and was reverted in full** |
| 24 | 11. Moonsighting research, session 2 | `ai/prompts/moonsighting-research-2.md` | `ai/plans/11-moonsighting-research-2/PLAN.md` | NOT PLANNED, deferred until further notice (owner 2026-09-18; runs after the SDK 58 programme and the deferred features) | | everything above |
