...
| 26 | 23. SDK 58 preview.7 + RN 0.88.0-rc.2: ride the beta to its latest | The owner's instruction of 2026-09-26 (🐋  "upgrade to 88 and an SDK 58 beta latest preview... regardless of if they are released candidates or not") | `ai/plans/23-sdk58-preview7/PLAN.md` | **NEEDS REPLAN: steps 1 to 3 shipped and are green, but the Android widgets render `undefined is not a function` on the 3T from step 1 onward.** Work continues on branch `fix/widget-render-rollback-20260926`. The bisect proved React 19.3.0, RN rc.2 and Reanimated 4.7.0 are NOT the cause (1.28.35, preview.7 with the old React and Reanimated, breaks identically), so the fault is in `expo-widgets`/`@expo/ui` 58.0.3 to 58.0.7. **Owner ruling 2026-09-26: roll those two packages back one version at a time until the widgets work, rather than fixing forward.** The ladder is 58.0.4, then .5 (where `cornerRadius` landed, the only change found in the whole Android render path), then .6, climbing from the last known-good so the first failure names the breaking version and every earlier build leaves the phone working. Everything else stays at preview.7. `LOG.md` holds every ruled-out suspect and the one-element diagnostic build that splits the search. Alarms and notifications are healthy throughout; nothing is reverted | `2a1dc302` | nothing |
