...
| 21 | 22. Dependency freshness sweep: re-measure every non-SDK package and batch what has moved | `ai/plans/README.md` "How a dependency upgrade is split into sessions" | `ai/plans/22-dependency-freshness-sweep/PLAN.md` | IN PROGRESS. Needs NO device: a desk session of registry reads, bumps and `yarn validate`. The sweep was measured while planning on 2026-09-26: **28 of the 30 non-SDK packages are still at their absolute latest and two have moved**, `@types/node` 26.6.3 and `lint-staged` 17.6.0, so the plan is 2 steps, one package each. Babel 8 re-confirmed blocked by `babel-preset-expo`'s 36 Babel 7 dependencies, so it stays with row 18 | `55df8421` | 20 (session 21 must be DONE, because it is what makes this a sweep rather than the first pass) |
