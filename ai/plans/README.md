...
| 24 | 25. Android widget countdown: a free-ticking Chronometer, so it can never drift (OPPO Find X8) | `ai/ISSUES.md` (to be filed) | not written yet | NOT PLANNED | | nothing. **OWNER RULING 2026-09-25, taken before planning: replace the pushed countdown text with Android's native `Chronometer` and accept `HH:MM:SS`.** 🐋  "it is very, very crucial that the widgets stay in sync. Otherwise they are absolutely useless to everyone", and 🐋  "I am willing to give up the format, if there's no other way." This is the same trade the owner already took on iOS in session 16a, where `3h 50m` became Apple's colon clock in exchange for a zero-entry self-ticking countdown, so the two platforms converge rather than diverge. `setChronometerCountDown(true)` is ticked by the LAUNCHER's process with zero app alarms, which makes drift structurally impossible instead of merely smaller. Three constraints the planner must design around, all verified 2026-09-25: (1) session 18 already measured that a `Chronometer` can only render `06:08:32`, recorded in `ai/AGENTS.md`, so the `6h 8m` shape and a free ticker are mutually exclusive on Android; (2) a `Chronometer` counts to a target and cannot render `--:--`, the stale card or a past-horizon state, so the layout branches to a plain `Text` for every non-live state; (3) **expo-widgets cannot express it from JS**: `ExpoWidgetEmittableTree.kt` converts 14 view types and has no Chronometer and no RemoteViews passthrough, while Glance itself DOES ship `AndroidRemoteViews` (confirmed in the resolved `glance-appwidget-1.2.0-peek-0.3.0.aar`), so the work is native Kotlin in `modules/widgetrefresh` or a converter patch, never a layout edit. The minute chain still has a job after this (the day rollover and the times), just not per-second accuracy. **Needs the Find X8.** The bug being fixed, observed by the owner: a medium widget left placed for a few hours read `8h 25m` while the true remaining was `8h 1m`, so it was **about 20 to 24 minutes late, and late always in the same direction**. Widgets only: notifications arm their own AlarmManager alarms per prayer and are unaffected (owner's own read, confirmed by the code paths being separate). Diagnosis from the source, unverified on the phone: `modules/widgetrefresh` arms ONE self-re-arming exact alarm, and `WidgetRefreshReceiver` computes the next edge from `System.currentTimeMillis()` at the moment it actually runs (`WidgetRefreshScheduler.armNext`). So a fire that ColorOS defers does not resume on the wall-clock grid, it **restarts the grid late and keeps the loss forever**; every later deferral adds to it, which is exactly a monotonically growing lateness. Three compounding causes to confirm: (1) no catch-up, the label is recomputed only by a render, so a missed render leaves a stale label; (2) `setExactAndAllowWhileIdle` is rate-limited to about one fire per minute per app in doze, and a per-minute chain sits exactly at that ceiling; (3) ColorOS 15 dozes background apps far harder than the 3T's Android 9, where the chain measures a clean 1232 alarms to 1232 wakeups with the next armed on the exact edge. Candidate fix for the planner to weigh: arm the next alarm from the previous TARGET edge rather than from `now`, so a late fire still lands the next one on the grid, plus a re-render whenever the computed label differs from the one last drawn. **Needs the Find X8, which is the owner's phone and was the second Android device these widgets ever ran on** |
