...
4. **Tests first (red).** This step is only tests, so red is measured by breaking what they guard rather than by the
   ...
   | `rides a Glance API that still exists` | A Glance upgrade that removed or renamed the function would break the patch at compile time only, which no Jest run would catch | The converter source | It contains `import androidx.glance.appwidget.action.actionStartActivity\n`, with the trailing newline. The newline is load-bearing: a renamed import such as `actionStartActivityGone` still contains the shorter name as a prefix, so a bare substring assertion passes while the API it names has gone. The execution session measured exactly that: without the newline, part 7's second break reported NOT CAUGHT |
