...
| --- | --- | --- |
| `useCountdown.test.ts` | **Worse than recorded.** `:12` mocks `@/shared/time` and `:32` requires `getSecondsBetween` back out of its own mock, so it asserts on the mock. Running it alone leaves `shared/time.ts` at **0% coverage**. It never imports the hook either. Four tests that can never fail for any production reason; the real helper is genuinely covered at `shared/__tests__/time.test.ts:461-476` with the same three cases. | Vacuous and redundant. Replaced in 1.27.5 by tests of the hook itself |
...
| Check | Result |
| --- | --- |
| Tests | 3,420 in 57 suites, green in London and under New York, Tokyo, Kiritimati and Pago Pago (`yarn test:tz`) |
...
- **Re-verified after the review-fix rounds**, on a build whose app code is `eb370a2`: Monday 14:00, Wednesday 06:00
  and Friday 10:00 matched the earlier capture on both pages, apart from a one-minute read-timing difference in
  Wednesday 06:00's countdown and ago badge; the 00:00 hand-over again moved Friday to Saturday in
...
- **What the device did not show:** the later commits change only the resume re-arm after a sync that stores days
  and the retry of a stalled 31 December request, plus comments. Keeping usable days on screen after a failed
  refresh is in the tested build, but this mock cannot trigger it. The mock answers at once and fails only past its
  last day, where nothing usable is stored, so the 31 December retry and the kept-days path cannot be reached on it.
  The resume re-arm could be, by moving the clock back before a resume, but was not run. All three are proven by the
  unit tests.
...
- A day whose last row is unreadable moves on after its last readable row, the same rule as every other day and
  as session 7's.
- R11: every row of a fully unreadable day is bright. The countdown first counted to the next readable prayer;
  the owner's ruling below replaced that.
...
- New, from the device: a tap on a disabled bell reached its row and opened that prayer's overlay. Superseded in 1.27.1:
  the bell opens the alert sheet with its message. In the overlay of a passed unreadable row, the bell belongs to the
  next readable occurrence and opens the usual options.
- **1.27.1, owner rulings of 2026-09-14:**
  - A fully unreadable day comes on screen at its own 00:00, not at the day before's last row.
  - The countdown shows `--:--` while the list on screen has no readable time left to come, with the bar
    and the "ago" badge hidden.
...
# Session 16a of the queue: iOS widgets, containerBackground to verdict, 2026-09-20

The black widgets were the archive budget, not the modifier: about 372 timeline entries per kind against WidgetKit's ~30 MB ceiling, masked by iOS as "Please adopt containerBackground API", with the missing `EXPO_PUBLIC_WIDGETS` flag as the first cause (both fixed on `wip/16a-ios-widget-archive-budget` through 1.27.312; landed on `uat-2` by step 1 of this plan's execution, merge `61346fa7`). This execution finished the row: all four Lock Screen kinds centre via `containerRelativeFrame({ axes: 'horizontal' })` with `multilineTextAlignment('center')` on the ticking digits (no Spacers, owner ruling; the tick centring and Layout 2's no-countdown simplification were owner rulings during the pass, 1.27.315 to 1.27.317); the dark kinds wear flat indigo-black `rgba(18, 14, 40, 0.95)` — the nebula orbs dropped by the owner's ruling after the real-device A/B isolated the orbs' runtime blur as the cause of the fresh-placement containerBackground flash (no patch + no orbs placed clean), and the expo-widgets memoisation patch, its postinstall hook and the patch-package devDependency went with them (1.27.318, 1.27.319); the countdown flips instantly on a 5-minute-gap spread mock (the 0:00 stall was the 1-minute mock); a freshly placed widget with the app closed shows content, not the placeholder (verified twice by the owner); the memoisation patch measured no benefit at this entry count (identical launch cycles: 0 CPU kills either side; the XS crash counter moved 9 to 12, all three at install-time push bursts, none in steady use); the Android side compiles and installs clean on the 3T at 1.27.319 — the owner then ruled the 3T down to the original-id app only (no fleettest), and the widget picker's disappearance on the fresh installs was diagnosed to `EXPO_PUBLIC_ANDROID_WIDGETS` never being set in the local `.env` (the committed catalog documents it; without it the APK silently ships picker-less), fixed and owner-verified live. The PNG-orb bake experiment is explicitly gated on the owner's go-ahead as this session's tail, on its own branch.

# Session 19 of the queue: widget polish, the horizon, and the iOS flag, 2026-09-24

Four planned jobs, two of which closed their findings, plus a long owner-led tail that produced two reverts and the session's most useful lesson.

**Android's dark card now matches iOS (ISSUES 39, CLOSED).** iOS renders `rgba(2, 13, 38, 0.95)` over the wallpaper; Android's card is an opaque bitmap because Glance draws no rounded corners, and it carried `#252387`, a bright purple nobody had reconciled with the iOS palette. The brief asked for an audit of the whole dark palette rather than the card alone: the generator holds 11 colour literals and exactly two sit outside the layout palette, both the deliberate opaque card forms, so the card was the only drift. Every pill fill, pill stroke and moon colour was already byte-identical. The owner then softened the whole dark theme (below), so what shipped is `rgba(9, 21, 47, 0.95)` on both platforms with `#09142d` baked for Android.

**The Android active pill wraps its row text (ISSUES 40, CLOSED).** The pill filled the list column while the rows sat 12dp inside it, so all the slack landed right of the times: 21dp at a 310dp grant, 29dp at 380dp, worse as the screen widened. The pill column now takes the dead space as a leading inset and the rows take that inset plus one 12dp gutter; where a narrow grant cannot afford 12dp each side, both sides take half the slack, which keeps them equal and keeps the pill inside its column. Verified against the real renderer at nine grants from 200 to 560dp, and measured on the 3T at 34px left and 34px right.

**The horizon is 7 days.** `TIMELINE_DAYS` feeds both platforms through `buildSequence`. Measured with the real builders: 7 days is 40 iOS entries and 17KB, where 30 was 178 and 75KB. The entry bounds moved from 250 and 500 to 60 and 60, because a bound sized for 30 days guards nothing at 7, and both stay literals: a bound derived from the constant would follow the horizon upward and notice nothing. Accepted risk, recorded plainly: a user whose Background App Refresh is off, or who force-quits, has a widget that goes stale after a week instead of a month. The owner took that trade deliberately, because survival without a refresh is the horizon's whole job.

**The iOS widgets flag is NOT flipped.** Its gate is the G.1 acceptance protocol on the iPhone XS, and the protocol needs the owner to place eight widgets and leave them ten minutes. The Release build is installed on the XS with the widget extension bundled (`ExpoWidgetsTarget.appex` present, which proves the flag reached the build) and the crash-report baseline is recorded at 15 `ExpoWidgetsTarget.cpu_resource` files, newest 2026-09-20. The next session runs the protocol without rebuilding. `shared/flags.ts` and `.env.example` are untouched.

**The owner-led tail, and what it cost.** The owner asked mid-session for full value-for-value parity with the iPhone, then for square smalls and full-width mediums. Both were built, both were reverted on the owner's own device evidence, and the reasoning errors are the durable part.

DURABLE LESSONS. (1) **An alignment only acts inside the space its container occupies.** The medium's trio sat 17dp left of its half because its `Column` carried `horizontalAlignment='center'` with no width: a shrink-wrapped container centres nothing and parks at its parent's leading edge. One missing `fillMaxWidth()` read as a sizing problem on both themes and was cured by one modifier. (2) **An appwidget grid cell is TALLER than it is wide**, so a square-looking declaration yields a portrait box: declaring the smalls 110x110 produced tall rectangles, and the original 160x110 was a landscape declaration compensating for exactly that, which nothing had recorded. Read a granted size back from `dumpsys appwidget` (`min=(WxH)` is `dp << 8`) rather than inferring shape from the declaration. (3) **Android keeps each widget's box from the moment it was placed**, so a declaration change is invisible until the widget is removed and re-added; the emulator still measured a 172x98dp small after the new APK was installed. (4) **Metrics do not transfer between cards of different shapes.** iOS's values are tuned for a 329x155pt card and the Android medium is 380x110dp, so copying paddings and row heights moved the content away from where it had been tuned to sit. The platform-specific tuning that looked like drift WAS the shape difference; colour transferred because colour is shape-independent. (5) **A widget's stored snapshot outlives an install**: the card bitmap updated on a plain `APPWIDGET_UPDATE` broadcast, but the text colours needed the app relaunched to push a fresh snapshot.

Checkpoint discipline: `checkpoint-1-android-good` tags `068d85d7`, the state the owner called perfect, with its APK and screenshot kept under `~/athan-device-sweep/session19/`. Both reverts returned to it, the second one byte for byte, confirmed by regenerating the PNGs and diffing against the tag.

# Session 20 of the queue: the widget horizon and the G.2 investigation, 2026-09-25

Two jobs, no behaviour a person can see.

**The horizon is 3 days.** `TIMELINE_DAYS` drops from 7 on the owner's ruling of 2026-09-25. Measured with the real builder: 3 days emits 23 iOS entries and 9,771 bytes per standard kind, where 7 emitted 47 and 20,183. The blackout that made entry count matter at all was ~380 entries, so both sit far below it. The horizon's job is how long a widget stays correct with no app launch and no background refresh; the owner judged three days enough because the background task runs every 3 hours and the foreground refresh every 2, so a phone in ordinary use re-pushes dozens of times inside the window. The volume guard is now the literal 22, what a 4-day horizon emits in that test's own fixture, so a creep back up fails the suite; the payload guard stays at 200KB, a comfort ceiling rather than a horizon-derived bound. ACCEPTED RISK, owner-approved and put to them twice before they confirmed: a phone whose background refresh has stopped entirely (an iOS force-quit, the ISSUES #36 reboot case, a long-idle install) shows the designed "Out of date" card after 3 days instead of 7.

**G.2 investigated, no code change.** The owner asked for a light look at the blank card seen at placement, and judged it probably a fluke. Findings, all read from the installed expo-widgets 58.0.3:

1. The G.1 root cause is GONE. `DynamicView.swift` holds no `UUID()` call; children take a stable string identity from `__expoWidgetIdentity`, which the widget bundle sets from the JSX type and key, and `expo-modules-core`'s `AnyChild.swift` resolves to `.string(...)`. That is expo/expo#49810, shipped in 58.0.1. `shared/flags.ts` had told the next reader the opposite ("merged but UNRELEASED", "installed 57.0.18 still carries the random-UUID identity hack"), and row 16 reads that text to decide whether its flip condition is met, so the stale claim was a live hazard rather than a typo. Corrected in the same session.
2. Entry count is not the driver. See the figures above: ~8x under the failure point before this session's change, and ~16x under it after.
3. What a freshly placed widget actually pays is the extension's cold start. A new extension process evaluates a 153,055-byte JavaScript bundle in JavaScriptCore, then evaluates and caches the layout string, before it can draw. No code in this repository runs before that, and the delay also appears while previewing in the picker, where no placement timeline exists yet, which is consistent with a cold start and inconsistent with a timeline-delivery explanation.
...
unchanged" is only knowable by checking, and the check is what stops a one-day drift becoming a migration. The scope
is every package NOT in `expo/bundledNativeModules.json` and not `expo`, an `expo-*`, an `@expo/*`, a
...
whose task rewrites files it was not handed (`() => "prettier --write ."`). It cannot reach this project: both staged
tasks take the staged filenames as arguments and write only those files, and the repo has no snapshot tests, the
other common source of a task touching an unstaged file. The owner was offered the `--hide-unstaged` guard and
declined it, so `.husky/pre-commit` is unchanged. 🐋  "I just want to be on the latest packages." (owner, 2026-09-26)

**Babel stays at 7, re-measured rather than inherited.** `babel-preset-expo@58.0.3` still depends on 36 Babel 7
packages and `@react-native/babel-preset@0.88.0-rc.0` still pins `@babel/core ^7.25.2`, so the four `@babel` packages
at 8.x remain blocked upstream and stay with row 18, the SDK 58 stable re-pin. Session 21 recorded this blocker; this
session re-checked it against the installed tree rather than trusting the record, which is the point of a sweep.

**The SDK-pinned set stays too.** `npx expo install --check` reports 18 outdated packages on 2026-09-26; every one is
SDK-pinned or deliberately ahead (`jest`, `@types/jest`, `typescript`, per `ai/AGENTS.md`). All 18 are row 18's
report, not this session's.

**DURABLE LESSON: a hook that starts a tool is not a hook that exercised it.** This plan claimed its own commit would
prove `lint-staged` still works, and that the log would show "two ticked task lines". It could not: the step stages
`package.json`, `yarn.lock`, `app.json` and markdown, and none of those match the tasks' glob
`**/*.{js,jsx,ts,tsx,mjs}`, so the hook printed `lint-staged could not find any staged files matching configured
tasks`. That proves lint-staged STARTED, and nothing more. Proof came from running it directly against a file that
does match (append a newline to `shared/logger.ts`, stage it, `npx lint-staged`), which ticked both tasks and then
exited non-zero with `prevented an empty git commit`, correct because Biome had stripped the newline again. Two
traps in one finding: a log line read for the wrong meaning, and a non-zero exit that is the healthy outcome. This is
session 21's lesson recurring in a new shape, so it is written here rather than left in a `LOG.md`.

**Verified:** `Test Suites: 170 passed, 170 total`, `Tests: 4662 passed, 4662 total`, 100% on statements
