...
MUTATIONS = [
    ...
    ('shared/sequence.ts', 'if (!isReadable(previous) || previous.datetime >= next.datetime) return null;', 'if (previous.datetime >= next.datetime) return null;', 'bar measured from a dashed row above'),
    ('shared/sequence.ts', ' || (position > 0 && listPosition(previous) !== position - 1)', '', 'previous taken further up past a missing row'),
